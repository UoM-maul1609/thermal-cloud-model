	...
	! locals
	real(wp), dimension(kp,ip) :: fx_r, fx_l, fz_r, fz_l
	integer(i4b) :: i,k
...
!$omp end simd
	...
	! locals
	real(wp) :: u_div1, u_div3, u_j_bar1, u_j_bar3, &
	...
	real(wp), dimension(-r_h+1:kp+r_h,-l_h+1:ip+r_h), target :: & 
		u_store1, u_store2
	real(wp), dimension(-l_h+1:kp+r_h,-r_h+1:ip+r_h), target :: &
		w_store1, w_store2
...
! 														wt,dims,coords)
		...
		call first_order_upstream_2d(dt,dxn,dzn,rhoa,rhoan, &
				ip,kp,l_h,r_h,ut,wt,psi_old,neumann)
		!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! 									psi_old,dims,coords)
	...
	subroutine mpdata_vec_2d(dt,dx,dz,dxn,dzn,&
						rhoa,rhoan, &
						ip,kp,nq,l_h,r_h,u,w,psi_in,kord,monotone,neumann)
#endif
	use numerics_type
#ifdef MPI
	use mpi_module
	...
	real(wp), dimension(-l_h+1:kp+r_h,-r_h+1:ip+r_h), &
		intent(in), target :: w
	real(wp), dimension(-r_h+1:kp+r_h,-r_h+1:ip+r_h,1:nq), &
		intent(inout), target :: psi_in
	real(wp), dimension(-l_h+1:ip+r_h), intent(in) :: dx, dxn
	real(wp), dimension(-l_h+1:kp+r_h), intent(in) :: dz, dzn, rhoa,rhoan
	logical, intent(in) :: monotone
	...
	real(wp), dimension(-r_h+1:kp+r_h,-l_h+1:ip+r_h), target :: & 
		u_store1, u_store2
	real(wp), dimension(-r_h+1:kp+r_h,-r_h+1:ip+r_h), target :: &
		v_store1, v_store2
	real(wp), dimension(-l_h+1:kp+r_h,-r_h+1:ip+r_h), target :: &
		w_store1, w_store2
...
! 														wt,dims,coords)
		...
		do n=1,nq
            call first_order_upstream_2d(dt,dxn,dzn,rhoa,rhoan, &
                    ip,kp,l_h,r_h,ut,wt,psi_in(:,:,n), neumann)
            if((it <= kord) .and. (kord >= 1)) then
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ! set halos														    	 !
                !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!		
